...
`normalize`는 지식데이터베이스 조회용 색인 `data/normalized/cspec_evidence_index.jsonl`도 만듭니다. `cspec_id` 하나당 한 줄이며, 다음 필드를 담습니다:

- `gene_symbols`: 그 문서가 다루는 유전자 목록
- `applicable`: `applicability`가 "적용 가능" 계열인 판정기준/강도 조합(`PM1_Strong`, `PM2_Supporting`처럼 코드+강도를 언더스코어로 이은 토큰; 강도가 없는 코드는 접미사 없이 코드만). 각 항목에는 `description`(해당 코드의 일반적인 ACMG/AMP 의미)과 `specification`(그 강도에 대한 VCEP별 구체적 적용 지침, 원본에 있는 경우)이 함께 들어갑니다.
- `not_applicable`: 같은 토큰 형식으로, 해당 VCEP가 명시적으로 쓰지 않는 조합. 원본 데이터상 이런 항목은 설명 텍스트가 아예 없어서 그냥 목록으로만 둡니다.
...
`--criteria-input`에는 `normalize` 단계가 만든 `cspec_criteria.jsonl`을 지정해야 합니다. 이 옵션을 주지 않으면 `--input`의 문서 JSONL만으로는 실제 판정기준 내용이 없기 때문에 `cspec_rules.jsonl`이 빈 파일로 생성됩니다(문서마다 `NO_RULE_CONTENT_FOUND` 경고 발생). 이는 버그가 아니라 판정기준 소스가 주어지지 않았다는 신호입니다.
